...
// 获取句子
function getNoThisSentence(elements, text) {
	if (!elements || elements.length <= 0) {
		return []
	}
	let a = text
	while (a == text) {
		a = getRandomElement(elements)
	}
	return a
}
...
function handlerSuccess(response) {
	...
	$("#sentence_new").click(function(){
		$("#sentence_new").fadeOut(speed-1000)
		$("#img-box").fadeIn(speed+1500)
	})

	// 淡入淡出效果
	$("#img-box").click(function() {
		$("#img-box").fadeOut(speed-1500)
